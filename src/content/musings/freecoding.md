...
date: 2025/06/08
description: 'A manifesto against vibe coding'
timeToRead: 15 min
image:
...
society, and especially on that of tech workers. Actually, let's home in on our
...
It's almost as if the very neurons that power our innovation are getting eaten
by these monsters of nondeterministic, predictive prowess. The synapses in our
collective conscious are devoured by these trillion-dollar beasts, all while we
are **FOOLED** by guises of progress, by that of pure laziness and willful
incompetence, and an ass-backwards refusal of learning[^ramblings].

This all sounds the ramblings of a man who's gone slightly insane, right? I
think I have in this regard, but for damn good reason. Many tech workers have
utilized AI in this way, and justified the terrible tradeoffs that this brings
about by quoting "efficiency" and "productivity"; they are collectively paying
the price of not learning for some minimal, short-lived success.

Oh my, Jared[^jared], look at the new application that's on the front page of
...
thoughts and contrived delusions of grandeur!
...
the best environment for this sort of style of coding, if you can call it that.
The goal? Shit out a proof-of-concept product over a weekend that does something
that would have otherwise taken a much longer time to spec out code for.
...
if Jesus himself had come down and enacted Isaiah 6:8-10 on just
programmers[^isaiah].
...
However, I decided to go one step further. I asked myself this one question:
...
and just fucking doing it themselves. Hardcore as fuck, and I love it.
...
This next-token technology has existed for a while in the form of other
precursors, such as [Markov chains](https://en.wikipedia.org/wiki/Markov_chain).

However, this means that no real semantic information is being encoded about
tokens, but rather LLMs will be learning from how tokens look like when
...
Let's take this concern, and apply it mathematically. Since an LLM is a token
_predictor_, it has some nonzero chance of generating incorrect predictions.
Let's see how this can spiral during a prompt-based conversation; assume a flat
per-token accuracy rate of 99.99% over a wide variety of situations, so a 0.01%
chance of error for the next token. This is probably more generous than I can
give most LLMs credit for, but I digress. Over a relatively short 500-token
conversation, the expected number of incorrect tokens within a binomial
distribution model is:

$$
E[\text{errors}] = n * p = 500 \times 0.0001 = 0.05
$$

A 0.05 expected error count does not sound bad at all, though? Well, there's a
catch: errors are not independent events, and can compound! LLMs condition their
output based on previous tokens, so a cumulative risk calculation for this is
more appropriate. Based on this approach, the new calculation for the
probability of at least one error in these 500 tokens is now this:

$$
P(errors >= 1) = 1 - (1 - p)^n = 1 - (1-0.0001)^{500} \approx 0.04877 \approx 4.9\%
$$

4.9%! Whoa! That’s nearly a 5% chance of encountering at least one incorrect
token in just 500 tokens, and many real conversations or code/comment blocks
easily exceed that. This is solely due to sporadic errors, though; errors can
result from bad prompting, bad context or training data, and poisoned prompts
from previous mistakes, so there's no telling to which the extent to which LLM
output is meaningful. At most, in the context of code, it's either "It works!"
without any questions or "Why no work?"[^why-no-work] without any proper
debugging, and both are bad in their own ways.
...
something broke because it was a `>` rather than a `>=` sign! Because the LLM is
so smart, apparently.
...
work?" and nothing else in the brain (cue: aka vibe coders), they run into
another problem. Their tests might just straight-up be wrong, and it'll slip
right past their empty brain and into code that I'll have an aneurysm looking at
two years later.
...
To be quite frank, I probably would have fallen into the same crevice, had I not
...
[^ramblings]:
    I know I'm starting to sound Nietzschean, but bear with me here. Simply put,
    I can't really stand here anymore and claim that the people that have fully
    embraced AI are somehow in the right anymore. IMO, the usage of AI in such a
    way that it becomes core to your workflow is a disservice to your own
    cognitive abilities, and that's not something I can excuse as just a valid
    "tradeoff".
...
[^insanity]:
    The "definition of insantity" as mentioned here is "doing the same thing
    over and over again and expecting a different result"; this is often
...
[^isaiah]:
    ...
    perceiving.’ 10Make the heart of this people calloused; make their ears dull
...
[^invalid-syntax]:
    ...
    invalid despite being in the latest version.
...
[^the-fool]:
    A reference to Psalm 14:1: "The fool says in his heart, there is no God!"
